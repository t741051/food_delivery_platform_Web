        ...
        <header>
	        <div class='container'>
				<nav class="navbar navbar-expand-lg navbar-light">
				  <a class="navbar-brand" href="deliveryman_index.php"><img src='pictures/logo.png' width='155'></a>
				  ...
				  <div class="collapse navbar-collapse" id="navbarSupportedContent">
				    <ul class="navbar-nav mr-auto">
				      <li class="nav-item">
				        <a class="nav-link" href="deliveryman_index.php">主頁</a>
				      </li>
				      <li class="nav-item">
					...
					<a class="nav-link"><?php echo '使用者名稱(貨運端) : ' . $_SESSION['id'];?></a>
				  </div>
				</nav>
	        </div>
        </header>
                ...
                <div>
					<table class="table" id="dynamic-table">
						...
						<?php 
                            ...
                            $delivery_take_order_id = $_SESSION['delivery_take_order'];

							$sql = "SELECT * FROM food_order,order_status,restaurant_attribute
							WHERE food_order.order_status = order_status.order_status
							AND	food_order.restaurant_id = restaurant_attribute.restaurant_id	
							AND	food_order.order_id = '$delivery_take_order_id'			 
							ORDER BY order_id ";
							//Execute the Query
									...
									while($rows = mysqli_fetch_assoc($res)){
											?>											
												<tr>
													...
													<tr>
														<td>													
															<a href="deliveryman_take_order_arrive.php?order_id=<?php echo $rows['order_id']; ?>" class="btn">已抵達</a>  
															<a href="deliveryman_take_order_finish.php?order_id=<?php echo $rows['order_id']; ?>" class="btn">完成訂單</a>  
														</td> 
													</tr>
												</tr>
											<?php										
...
/*
							...
							$sql = "SELECT * FROM food_order,restaurant_name 
                                    WHERE food_order.restaurant_id = restaurant_name.restaurant_id AND food_order.order_id = '$delivery_take_order_id'";
							//Execute the Query
							$res = mysqli_query($conn, $sql);
							//CHeck whether the Query is Executed of Not
							if($res==TRUE){
									...
									while($rows = mysqli_fetch_assoc($res)){
                                        ?>											
                                            <tr>
                                                ...
                                                <td><?php echo $rows['food_amount'];?>  </td>
                                                <td><?php echo $rows['address'];?>		</td>
                                                <td><?php echo $rows['phone'];?>		</td>
                                                <td><?php echo $rows['money'];?>        </td>                                                                                         													
                                            </tr>
	        ...
	        <div class="container">
		        <div class='row'>
					  ...
					  <div class='col-md-4 text-left'>
						  <h4>連結</h4>
						  <ul>
					      	<li><a href='deliveryman_index.php'>主頁</a></li>
